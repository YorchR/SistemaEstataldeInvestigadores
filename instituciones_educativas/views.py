...
def instituciones_educativas_lista(request):
    ...
    instituciones = InstitucionEducativa.objects.filter(encargado__aprobado=True)

    if query:
        instituciones = instituciones.filter(
            Q(nombre_institucion__icontains=query) |
            Q(especialidades__nombre__icontains=query) |
            Q(area_conocimiento__nombre__icontains=query)
        ).distinct()
    ...
    paginator = Paginator(instituciones, 5)
