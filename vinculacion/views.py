...
from investigadores.forms import SolicitudTrabajoForm, FormInvestigacion
from usuarios.models import TipoUsuario
from vinculacion.models import Categoria, Noticia
from administracion.models import Mensaje
...
def ver_mensajes(request):
    ...
    paginator = Paginator(mensajes_recibidos, 5)
    numero_pagina = request.GET.get('page')
    page_obj = paginator.get_page(numero_pagina)
    mensajes_recibidos.update(leido=True)

    return render(request, 'vinculacion/mensajes.html', {'page_obj': page_obj})

@login_required
def mark_as_read(request, message_id):
    message = Mensaje.objects.get(id=message_id, destinatario = request.user)
    message.leido = True
    message.save()
...
class InvestigacionDetalles(DetailView):
    model = Investigacion
    template_name = "vinculacion/investigacion_detalles.html"
...
class MensajeDetalle(DetailView):
    model = Mensaje
    template_name = 'vinculacion/mensaje_detalle.html'
    context_obj_name = 'mensaje'

    def get_object(slef, queryset = None):
