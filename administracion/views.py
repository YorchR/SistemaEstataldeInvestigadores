...
@user_passes_test(user_is_staff_member)
def aprobar_institucion(request, pk):
    usuario = User.objects.get(pk=pk)
    usuario.aprobado = True
    usuario.save()
    messages.success(request, "Solicitud aceptada")
...
class InvestigadorDetalles(DetailView):
    model = Investigador
    template_name = "administracion/investigadores_detalles.html"
    context_object_name = 'investigador'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        investigador = self.get_object()
        context['tiene_imagen'] = investigador.imagen
        return context
    
class EnviarMensaje(CreateView):
    model = Mensaje
    form_class = MensajeForm
    template_name = 'administracion/enviar_mensaje.html'
    ...
    def get_initial(self):
        destinatario_id = self.kwargs['pk']
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context['unread_count'] = Mensaje.objects.filter(destinatario = self.request.user, leido = True).count()
        return context
...
class EmpresaDetalles(DetailView):
    model = Empresa
    template_name = "administracion/empresa_detalles.html"
    context_object_name = 'empresa'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        empresa = self.get_object()
        context['tiene_imagen'] = empresa.imagen
        return context
...
class InstitucionEducativaNuevo(UserPassesTestMixin, CreateView):
    ...
    template_name = "vinculacion/formulario_institucion.html"
    extra_context = {"accion": "Crear",
...
class InstitucionEducativaDetalles(DetailView):
    model = InstitucionEducativa
    template_name = "administracion/institucion_educativa_detalles.html"
    context_object_name = 'institucion_educativa'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        institucion_educativa = self.get_object()
        context['tiene_imagen'] = institucion_educativa.imagen
        return context
...
def enviar_mensaje(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mensaje.save()
            return redirect('#') ##Ruta donde se encuentra la ventana 
